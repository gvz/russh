...
#[derive(Parser, Debug)]
#[clap(name = "echoserver_custom_keys", about = "Echo server with custom keys")]
struct Cli {
...
async fn main() {
    ...
    if let Some(cert_path) = args.cert {
        let cert = russh::keys::load_openssh_certificate(&cert_path).expect("Could not load certificate");
        certs.push(cert);
    }
