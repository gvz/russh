    ...
    pub async fn step<H: Handler + Send>(
        ...
        match self.state {
            ...
            ServerKexState::WaitingForDhInit { mut names, mut kex } => {
                ...
                let (key, certificate) = if let Some(cert) = self
                    .config
                    .certificates
                    .iter()
                    .find(|c| c.algorithm().to_certificate_type() == names.key.to_string())
                {
                    let key = self
                        .config
                        .keys
                ...
                let signature_hash_alg = match &names.key {
                    ...
                    Algorithm::Other(name) => {
                        if name.as_str().starts_with("rsa-sha2-256-cert") {
                            Some(ssh_key::HashAlg::Sha256)
                        } else if name.as_str().starts_with("rsa-sha2-512-cert") {
